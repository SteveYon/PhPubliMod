...
echo "DOCUMENT_EDIT <br>\n";
	...
	if ($mode=="editdocument")
	{
		...
		foreach ($list_typedoc as $id=>$name)
		{
			...
			if ($id==$document->typedoc_id) echo " selected";
			echo ">$name</option>\n";
		}
		echo "</td>\n";
		echo "</tr>\n";
...
function document_form($mode, $tab, $bd)
{
	...
	if ($mode=="update")
	{
		...
		{
			echo "<option value=\"$id\"";
			if ($id==$tab->journal_id) echo " selected";
			echo ">$name</option>\n";
		}
		echo "</td>\n";
		echo "</tr>\n";
...
function document_data_fixed($doc_id, $bd)
{
	...
	if ("$typedoc_libelle"=="article")
	{
		...
		foreach ($list_journal as $id=>$name)
		{
			if ($id==$document->journal_id) echo "$name";
		}
		echo "</td>\n";
		echo "</tr>\n";
	}
...
//MODIFIER
		...
		{
			echo "<option value=\"$id\"";
			if ($id==$document->lang) echo " selected";
			echo ">$name</option>\n";
		}
		echo "</td>\n";
		echo "</tr>\n";
...
function document_data_update($document, $bd)
{
	...
	$query = "UPDATE document SET "
		...
		. "soustypedoc_id='$soustypedoc_id', "
		. "title='$title', year='$year', institution_id='$institution_id', volume='$volume', "
		. "pages_start='$pages_start', pages_end='$pages_end', pages_eid='$pages_eid', pages_num='$pages_num', "
		. "doi='$doi', hal='$hal', "
		. "journal_id='$journal_id', "
		. "conference_id='$conference_id', "
		. "proceedings_id='$proceedings_id', "
		. "publisher_id='$publisher_id', "
		. "note='$note', groupe='$groupe', lang='$lang', "
		. "log='1', date=now()  "
		. "WHERE doc_id = '$doc_id'";
	//print ("query= $query <p>\n");
	$res = $bd->exec_query($query);
...
function document_data_insert($document, $bd)
{
	...
	$query = "INSERT INTO document ( "
		. "typedoc_id, soustypedoc_id, institution_id, title"
		. ", year, volume, pages_start, pages_end, pages_eid, pages_num, doi, hal"
		. ", journal_id, conference_id, publisher_id, proceedings_id"
		. ", note, groupe, lang"
		. ", log, date "
		. ")  VALUES ( "
		. " '$typedoc_id', '$soustypedoc_id', '$institution_id', '$title'"
	...
	$query = "SELECT * "
		...
		. "AND year='$year' "
		. "AND volume='$volume' "
		. "AND pages_start='$pages_start' "
		. "AND pages_end='$pages_end' "
		. "AND pages_eid='$pages_eid' "
		. "AND pages_num='$pages_num' "
		. "AND doi='$doi' "
		. "AND hal='$hal' "
...
function document_update_old($action, $document, $bd)
{
	...
	if ($action=="updatedocument")
	{
		...
		$query = "UPDATE document SET "
			. "typedoc_id='$typedoc_id', "
			. "title='$title', year='$year', volume='$volume', "
			. "pages_start='$pages_start', pages_end='$pages_end', pages_eid='$pages_eid', pages_num='$pages_num', "
			. "doi='$doi', journal_id='$journal_id', note='$note', groupe='$groupe', "
			. "log='1', date=now()  "
			. "WHERE doc_id = '$doc_id'";
		//print ("query= $query <p>\n");
		$res = $bd->exec_query($query);
...
// ICI VALIDATE AUTHORS
	...
	{
		$query = "INSERT INTO document (title, year, volume, pages_start, pages_end, pages_eid, pages_num, doi, journal_id, note, groupe, log, date ) "
			. " VALUES ( '$title', '$year', '$volume', '$pages_start', '$pages_end', '$pages_eid', '$pages_num', '$doi', '$journal_id', '$note', '$groupe', '0', now()) " ;
		// print ("query= $query <p>\n");
		$res = $bd->exec_query($query);
		...
		$query = "SELECT * "
			...
			. "AND year LIKE '%$year%' "
			. "AND journal_id LIKE '%$journal_id%' ";
		// print ("query= $query <p>\n");
		$res = $bd->exec_query($query);
		$document_new= $bd->fetch_object ($res);
